...
pub(super) async fn sine_queue(case: SmoothingCase) -> (OfflineQueue<TestPools>, u64) {
    ...
    let harness = OfflineQueue::new(session, queue).expect("create offline queue");
...
async fn observe(harness: &OfflineQueue<TestPools>, blocks: usize) -> Vec<f32> {
    ...
    for _ in 0..blocks {
        harness.control().tick().expect("tick sine queue");
        pcm.extend(harness.render(Consts::BLOCK_FRAMES));
...
async fn deck_volume_step_is_ramped() {
    ...
    harness.control().set_volume(0.0);
