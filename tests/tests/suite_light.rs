...
// The nested `PlayerConfig -> PlayerSettings -> EngineSettings` builder chain
// (see `kithara-play/src/player/config.rs`) adds generic depth that, inside
// this crate's large aggregate of async test modules, pushes the type-layout
// query past the default limit for `run_crossfade_flac_case` and its kin.
#![recursion_limit = "256"]
