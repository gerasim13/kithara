...
use kithara::{
    audio::{
        AssetAxis, AssetFrame, BeatEvidence, BeatGrid, BeatMapId, BeatMapRevision, BeatMapSnapshot,
        BeatMarker, BeatOrdinal, FrameUncertainty, MapAxis, MapSegment, MapState, MapUnavailable,
        Meter, MeterFacts, SegmentFacts, SegmentSet, Waveform,
        analysis::{BeatAnalysisConfig, TrackAnalysis},
    },
    bufpool::{BytePool, PcmPool},
    hls::AbrMode,
    platform::{CancelToken, time::SystemTime, tokio::task},
...
pub fn analysis_map(analysis: &TrackAnalysis, media: &str) -> SyncFixtureResult<BeatMapSnapshot> {
    ...
    BeatMapSnapshot::try_from((id, BeatMapRevision::first(), MapState::Complete, segments)).map_err(
        |error| SyncFixtureError::IncompleteAnalysis {
            media: media.to_owned(),
...
pub fn unavailable_analysis_map(
    ...
    let axis = MapAxis::Asset(AssetAxis::new(source_rate, analysis.source_frames()));
    let segments = SegmentSet::new(axis, Vec::new()).map_err(|error| {
        SyncFixtureError::IncompleteAnalysis {
            media: media.to_owned(),
            detail: format!("unavailable map topology is unusable: {error}"),
        }
    })?;
    let id = BeatMapId::allocate().map_err(|error| SyncFixtureError::IncompleteAnalysis {
        media: media.to_owned(),
    ...
    .map_err(|error| SyncFixtureError::IncompleteAnalysis {
        media: media.to_owned(),
        detail: format!("unavailable map snapshot is unusable: {error}"),
    })
}
