...
fn sync_group_rows() -> Vec<OracleRegistration> {
    ...
    [
        ...
        (
            ...
            "add_remove_and_replace_prepare_one_atomic_topology_candidate",
            "ordered edits prepare one later immutable candidate",
        ),
        (
            "SYNC-GROUP-008",
            "active_transaction_owns_the_composition_topology",
            "rejected_update_leaves_the_published_snapshot_unchanged",
        ...
        (
            "SYNC-GROUP-011",
            "registration_advances_typed_topology_revision",
            "stale_update_base_leaves_the_published_snapshot_unchanged",
            "membership edits are stamped against one topology revision",
        ),
    ]
    .into_iter()
    .map(|(id, old, new, observable)| {
        frozen(FrozenRegistration {
            oracle_id: id,
            pr: Some(118),
            sha: PR_118_SHA,
