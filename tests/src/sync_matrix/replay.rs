...
impl SyncHarness {
    ...
    fn current_session_frame(&self) -> Result<i64> {
        let position = match self.session.exec(Cmd::QuerySessionTransport)? {
            Reply::SessionTransport(snapshot) => snapshot.position(),
        ...
        let anchor = match self.session.exec(Cmd::QuerySessionAnchor)? {
            Reply::SessionAnchor(anchor) => anchor
                .load()
                .context("session anchor has not been committed")?,
            Reply::Err(error) => bail!("{}: session anchor query failed: {error}", self.case),
            _ => bail!(
                "{}: session anchor query returned an unexpected reply",
                self.case
            ),
        };
        anchor
