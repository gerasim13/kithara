...
use kithara::{
    audio::{StretchControls, StretchKind, analysis::TrackAnalysis},
    ...
    play::{
        Cmd, PlayError, PlayerConfig, PlayerImpl, Reply, SessionDispatcher, SessionError, Tempo,
    },
    queue::{BeatQuantum, Queue, QueueConfig, QueueError, TrackBeat, Transition},
};
...
use crate::offline::OfflineSession;
...
impl SyncHarness {
    ...
    pub(super) fn apply(&mut self, deck_index: usize, operation: Operation) -> Result<()> {
        ...
        match operation {
            ...
            Operation::Sync => {
                ...
                let quantum = BeatQuantum::new(4.0).context("valid bar quantum")?;
                deck.queue
                    .start_at_beat(index, &deck.analysis, TrackBeat::default(), quantum)
    ...
    fn republish_analysis(&mut self, deck_index: usize, analysis: &TrackAnalysis) -> Result<()> {
        ...
        deck.sync_index = Some(index);
        deck.queue
            .start_at_beat(index, analysis, TrackBeat::default(), quantum)
    ...
    fn withdraw_analysis(&mut self, deck_index: usize) -> Result<()> {
        let deck = self
            ...
            .get(deck_index)
            .with_context(|| format!("{}: no deck {deck_index}", self.case))?;
        deck.queue.unbind_from_grid().with_context(|| {
            format!(
        ...
        let unavailable = match deck.queue.start_at_beat(
            index,
            ...
            Ok(()) => false,
            Err(QueueError::Play(PlayError::BindUnavailable { .. })) => true,
