...
use super::{
    COCHLEA_PHASE_SPREAD_BUDGET_FRAMES, CaptureBundle, LockedPhaseObservation,
    MAX_LOCKED_PHASE_ERROR_FRAMES, PcmCapture, RENDER_FRAMES, SyncCase,
};
use crate::{
...
const COCHLEA_PHASE_CALIBRATION_SHIFT_FRAMES: usize = 512;
...
impl SyncOracle {
    ...
    pub fn evaluate(case: SyncCase, bundle: &CaptureBundle) -> SyncOracleReport {
        ...
        failures.extend(exact_phase_failures(&bundle.facts.phase_observations));
...
fn exact_phase_failures(observations: &[LockedPhaseObservation]) -> Vec<String> {
...
mod tests {
    ...
    fn exact_stamp_oracle_rejects_one_frame_activation_shift() {
        ...
        assert!(
            failures
                .iter()
                .any(|failure| { failure.contains("activation stamp shifted by 1 frame(s)") }),
    ...
    fn exact_phase_oracle_rejects_one_beat_bar_phase_error() {
        ...
        assert!(
            failures
                .iter()
                .any(|failure| failure.contains("resolved beat ordinal")),
            "the exact phase oracle accepted a one-beat ordinal error: {failures:?}"
        );
        assert!(
            failures.iter().any(|failure| failure.contains("bar phase")),
