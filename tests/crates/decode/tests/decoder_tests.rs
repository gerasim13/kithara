...
use kithara_test_fixtures::fixtures::{tone_mp3, tone_wav};
...
fn decode_with_probe(
    ...
    let data = if use_wav { tone_wav } else { tone_mp3 };
...
fn from_media_info(
    ...
    let (data, info) = if use_wav {
        (tone_wav, &wav_media_info)
    } else {
        (tone_mp3, &mp3_media_info)
    };
    let reader = Cursor::new(data);
...
fn spec_properties(#[case] ext: &str, tone_wav: &'static [u8], tone_mp3: &'static [u8]) {
    let data = if ext == "wav" { tone_wav } else { tone_mp3 };
    let reader = Cursor::new(data);
