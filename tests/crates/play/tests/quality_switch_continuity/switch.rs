...
pub(super) const ORACLE_SLACK: f32 = 2.0e-3;
pub(super) const COCHLEA_WINDOW_MS: f64 = 20.0;

pub(super) const AAC_LOW: usize = 0;
pub(super) const AAC_HIGH: usize = 1;
pub(super) const FLAC: usize = 2;

pub(super) fn longest_silent_run(samples: &[f32]) -> usize {
...
/// untested.
pub(super) const TRANSITIONS: [Transition; 6] = [
...
impl Transition {
    pub(super) const fn new(label: &'static str, from: usize, to: usize) -> Self {
        Self { label, from, to }
    }
}

pub(super) struct PreparedPlayer {
    pub(super) _temp: TestTempDir,
    pub(super) player: OfflinePlayer,
    pub(super) abr: AbrHandle,
...
#[derive(Debug)]
pub(super) struct ControlRender {
    pub(super) samples: Vec<f32>,
    pub(super) capture_frame: i64,
}

#[derive(Clone, Copy, Debug)]
pub(super) struct DecoderObservation {
    pub(super) frame_end: usize,
    pub(super) backend: DecoderBackendKind,
    pub(super) cause: DecoderChangeCause,
    pub(super) codec: Option<AudioCodec>,
    pub(super) sample_rate: u32,
    pub(super) channels: u16,
    pub(super) variant: Option<u32>,
}

pub(super) fn fixture() -> HlsFixtureBuilder {
    fixture_with_signal(PackagedSignal::Sine { freq_hz: SINE_HZ })
}

pub(super) fn fixture_with_signal(signal: PackagedSignal) -> HlsFixtureBuilder {
...
pub(super) fn source_frame(position: f64, label: &str) -> i64 {
...
pub(super) fn observation_frames() -> usize {
...
pub(super) fn assert_initial_decoder(
...
#[kithara::flash(true)]
pub(super) async fn render_paced(player: &mut OfflinePlayer, frames: usize) -> Vec<f32> {
...
pub(super) async fn render_switch(
    master_url: &url::Url,
...
pub(super) async fn render_no_switch_control(
    master_url: &url::Url,
    initial_variant: usize,
    backend: DecoderBackend,
    label: &str,
