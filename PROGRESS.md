...
- Tooling parameter ownership. Every policy number and list `xtask` and
  ...
  shell. Cleanup pruned `target-slots`, every Linux job's
  `CARGO_TARGET_DIR`, as retired.
...
- Work the comment queue down by hand: `--fix` is exhausted, so all 667
  findings are decisions.
- 602 ordering findings are mechanical; one `just lint style --fix` clears
