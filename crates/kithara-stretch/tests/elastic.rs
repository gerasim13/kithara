...
use kithara_stretch::{
    ElasticCapabilities, ElasticConfig, ElasticError, ElasticRequest, SignalsmithBackend,
};
...
fn warmup_request(
    ...
    let source_frames = output_frames
        .to_f64()
        .map(|frames| frames * source_frames_per_output)
        .and_then(|frames| frames.round().to_usize())
        .expect("invariant: warmup span fits in usize");
