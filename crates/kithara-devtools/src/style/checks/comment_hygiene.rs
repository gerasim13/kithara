...
fn leading_license_range(src: &str, comments: &[Comment]) -> Option<Range<usize>> {
    let first = comments.first()?;
    if first.kind != CommentKind::Line
        || first.doc_style != DocStyle::None
        || !src[..first.byte_range.start].trim().is_empty()
    {
        return None;
    }
    ...
    for comment in &comments[1..] {
        if comment.kind != CommentKind::Line
            ...
            || !src[end..comment.byte_range.start].trim().is_empty()
        {
            break;
        }
        end = comment.byte_range.end;
    }

    let notice = &src[start..end];
    (notice.contains("Copyright")
        && (notice.contains("License") || notice.contains("SPDX-License-Identifier")))
    .then_some(start..end)
}
...
",
    ...
    #[test]
    fn leading_license_notice_is_preserved() {
    ...
    #[test]
    fn ordinary_leading_comment_is_still_flagged() {
        let src = "// Generated by an agent.\n\nfn f() {}\n";
        assert_eq!(keys(&run_all(src)), vec!["fixture.rs:1:category"]);
    }
