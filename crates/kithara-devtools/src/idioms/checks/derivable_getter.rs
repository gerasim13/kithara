...
impl AccessorKind {
    const fn label(self) -> &'static str {
        match self {
            Self::Get => "read",
            Self::GetMut => "mutable",
            Self::With => "owned chainable setter",
        }
    }
}
...
enum FieldworkMode {
    Concise { default_visibility: String },
    OptIn,
}
...
fn complete_type<'a>(
    ...
    for conversion in &converted {
        ...
        match conversion.raw.kind {
            ...
            AccessorKind::With => entry.with = Some(conversion),
        }
    }
...
fn plan_method(
    ...
    if kind == AccessorKind::With {
        ...
        if body == BodyKind::OptionSetSome
            && value_ty.is_some_and(|value_ty| option_inner(field_ty) == Some(value_ty))
            && is_self_type(return_ty)
        {
            return Ok(MethodPlan {
                adjustment: Adjustment::Default,
                option_set_some: true,
            });
        }
...
fn detect_with(method: &ImplItemFn) -> Option<DetectedAccessor<'_>> {
    let signature = &method.sig;
    let method_name = signature.ident.to_string();
    if method_name == "with_" || signature.inputs.len() != 2 {
        return None;
    }
    let FnArg::Receiver(receiver) = signature.inputs.first()? else {
        return None;
    ...
    if !expr_is_ident(tail, "self") {
        return None;
    }
    let argument_name = argument.ident.to_string();
    let body = if expr_is_ident(&assign.right, &argument_name) {
        BodyKind::Assign
    } else if is_some_of_ident(&assign.right, &argument_name) {
        BodyKind::OptionSetSome
    } else {
        return None;
    };
...
fn is_some_of_ident(expr: &Expr, ident: &str) -> bool {
    let Expr::Call(call) = expr else { return false };
    expr_is_ident(&call.func, "Some")
        && call.args.len() == 1
        && call
            .args
            .first()
            .is_some_and(|argument| expr_is_ident(argument, ident))
}
...
fn add_struct_attributes(
    ...
    let has_get = converted
        .iter()
        .any(|conversion| conversion.raw.kind == AccessorKind::Get);
    let has_mut = converted
        .iter()
        .any(|conversion| conversion.raw.kind == AccessorKind::GetMut);
    let has_with = converted
        .iter()
        .any(|conversion| conversion.raw.kind == AccessorKind::With);
    let mut methods = Vec::new();
    if has_get || has_mut {
        methods.push("get");
    }
    if matches!(mode, FieldworkMode::OptIn) && has_mut {
        methods.push("get_mut");
    }
    if has_with {
        methods.push("with");
    }
    ...
    let attribute = match mode {
        FieldworkMode::Concise { default_visibility } if default_visibility == "pub" => {
            format!("#[fieldwork({methods})]\n")
        }
        FieldworkMode::Concise { default_visibility } => {
            format!("#[fieldwork({methods}, vis = \"{default_visibility}\")]\n")
        }
        FieldworkMode::OptIn => format!("#[fieldwork(opt_in, {methods})]\n"),
    };
    insertions
        .entry(start)
        .or_default()
...
fn fieldwork_mode(strukt: &ItemStruct, converted: &[Converted<'_>]) -> FieldworkMode {
    ...
    let has_with = converted
        .iter()
        .any(|conversion| conversion.raw.kind == AccessorKind::With);
    let full_get = !has_get || has_full_coverage(strukt, converted, AccessorKind::Get);
    let full_with = !has_with || has_full_coverage(strukt, converted, AccessorKind::With);
    if (!has_get && !has_with) || !full_get || !full_with {
        return FieldworkMode::OptIn;
    }
    let mut visibilities = converted
        .iter()
        .filter(|conversion| matches!(conversion.raw.kind, AccessorKind::Get | AccessorKind::With))
        .map(|conversion| visibility_text(&conversion.raw.method.vis));
    let first = visibilities.next().unwrap_or_else(|| "pub".to_owned());
    let default_visibility = if visibilities.all(|visibility| visibility == first) {
        first
    } else {
        "pub".to_owned()
    };
...
fn has_full_coverage(strukt: &ItemStruct, converted: &[Converted<'_>], kind: AccessorKind) -> bool {
    let fields: BTreeSet<_> = converted
        .iter()
        .filter(|conversion| conversion.raw.kind == kind)
...
fn concise_field_config(methods: &FieldMethods<'_>, default_visibility: &str) -> Option<String> {
    ...
    if methods.with.is_some_and(|with| with.plan.option_set_some) {
        parts.push("option_set_some".to_owned());
    }
...
fn field_config(methods: &FieldMethods<'_>) -> String {
    ...
    let get_mut_vis = methods
        .get_mut
        .map(|value| visibility_text(&value.raw.method.vis));
    let with_vis = methods
        .with
        .map(|value| visibility_text(&value.raw.method.vis));
    let mut visibilities = [get_vis, get_mut_vis, with_vis].into_iter().flatten();
    let common_vis = visibilities.next().and_then(|first| {
        visibilities
            .all(|visibility| visibility == first)
            .then_some(first)
    });
    let mut getter_methods = [methods.get, methods.get_mut].into_iter().flatten();
    let common_deref = getter_methods.next().is_some_and(|first| {
        first.plan.adjustment == Adjustment::DerefFalse
            && getter_methods.all(|value| value.plan.adjustment == Adjustment::DerefFalse)
    });
...
mod tests {
    ...
    #[test]
    fn plain_with_setter_converts_in_opt_in_mode() -> Result<()> {
        let source = "struct Request {\n    url: String,\n    retries: usize,\n}\nimpl Request {\n    pub fn with_url(mut self, url: String) -> Self { self.url = url; self }\n}\n";
        let (fixed, outcome, _) = fix(source)?;
        assert_eq!(outcome.changes, ["with_url"]);
        assert_eq!(
            fixed,
    ...
    #[test]
    fn transformed_and_borrowed_with_setters_stay_manual() -> Result<()> {
        let source = "struct Values {\n    first: String,\n    second: String,\n}\nimpl Values {\n    pub fn with_first(mut self, value: &str) -> Self { self.first = value.into(); self }\n    pub fn with_second(&mut self, value: String) -> Self { self.second = value; self }\n}\n";
        let (fixed, outcome, findings) = fix(source)?;
        assert_eq!(fixed, source);
        assert_eq!(outcome.writes, 0);
        assert!(findings.is_empty());
    ...
    #[test]
    fn full_get_and_with_coverage_share_concise_configuration() -> Result<()> {
        let source = "struct Request {\n    url: String,\n}\nimpl Request {\n    pub fn url(&self) -> &str { &self.url }\n    pub fn with_url(mut self, url: String) -> Self { self.url = url; self }\n}\n";
        let (fixed, outcome, _) = fix(source)?;
        assert_eq!(outcome.changes, ["url", "with_url"]);
        assert_eq!(
            fixed,
            "#[derive(fieldwork::Fieldwork)]\n#[fieldwork(get, with)]\nstruct Request {\n    url: String,\n}\n"
        );
    ...
    #[test]
    fn with_method_can_be_renamed_independently_of_its_field() -> Result<()> {
        let source = "struct Request {\n    bar: String,\n    retries: usize,\n}\nimpl Request {\n    pub fn with_foo(mut self, value: String) -> Self { self.bar = value; self }\n}\n";
        let (fixed, outcome, _) = fix(source)?;
        assert_eq!(outcome.changes, ["with_foo"]);
        assert!(fixed.contains("#[fieldwork(opt_in, with)]"), "{fixed}");
        assert!(
            fixed.contains("#[field(with = with_foo)]\n    bar: String"),
            "{fixed}"
        );
        assert!(!fixed.contains("fn with_foo"), "{fixed}");
        Ok(())
    }

    #[test]
    fn with_setter_fix_is_idempotent() -> Result<()> {
        let source = "struct Request {\n    url: String,\n    retries: usize,\n}\nimpl Request {\n    pub fn with_url(mut self, url: String) -> Self { self.url = url; self }\n}\n";
        let (fixed, first, _) = fix(source)?;
        assert_eq!(first.writes, 1);
        let (again, second, _) = fix(&fixed)?;
