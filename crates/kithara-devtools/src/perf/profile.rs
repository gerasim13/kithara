...
pub(crate) fn run(params: &ProfileParams, project: &ProjectConfig) -> Result<()> {
...
mod tests {
    ...
    fn symbolicates_function_names_from_samply_sidecar() {
        ...
        let replaced =
            symbolicate_profile_json_value(&mut profile, &symbols).expect("symbolicate profile");

        assert_eq!(replaced, 2);
        let thread = &profile["threads"][0];
        let strings = thread["stringArray"].as_array().expect("strings");
        let names = thread["funcTable"]["name"].as_array().expect("names");
        let first_name =
            usize::try_from(names[0].as_u64().expect("first name index")).expect("usize");
        let second_name =
            usize::try_from(names[1].as_u64().expect("second name index")).expect("usize");
        assert_eq!(strings[first_name], "demo_mod::do_work");
