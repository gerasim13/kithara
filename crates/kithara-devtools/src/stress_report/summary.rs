...
pub(crate) struct StressReportArgs {
    ...
    #[arg(skip)]
    #[field(with, vis = "pub(crate)")]
    pub(super) render: StressRenderBudgets,
...
fn render_flakes(
    ...
    for ((suite, name), stats) in flakes.iter().take(budgets.failure_rows) {
        ...
        let _ = writeln!(
            ...
            render_iterations(&stats.flaky_iterations, budgets),
        );
    }
    if flakes.len() > budgets.failure_rows {
        let rows = budgets.failure_rows;
        let _ = writeln!(
            out,
            "\nShowing the first {rows} of {} retried tests. The JUnit artifact is exhaustive.",
            flakes.len()
        );
    }
}
...
pub(super) fn test_id(case: &CaseTiming, budgets: &StressRenderBudgets) -> String {
...
/// last.
pub(super) fn markdown_cell(text: &str, budgets: &StressRenderBudgets) -> String {
    let sanitized = text
        .replace('|', "\\|")
        .replace(['\r', '\n'], " ")
        .replace('`', "'");
