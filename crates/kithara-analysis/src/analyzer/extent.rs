...
mod tests {
    ...
    fn an_extent_is_measured_on_the_pass_axis() {
        ...
        extent.claim(None, rate());
        assert_eq!(
            extent.frames(),
            Some(88_200),
            "a live source reports no duration, which retracts nothing"
        );
        extent.claim(Some(Duration::ZERO), rate());
        assert_eq!(extent.frames(), Some(88_200));
    }
    ...
    fn a_reported_length_grows_and_a_proved_one_bounds_it() {
        let mut extent = Extent::default();
        extent.claim(Some(Duration::from_secs(2)), rate());
        extent.claim(Some(Duration::from_secs(4)), rate());
        assert_eq!(
            extent.frames(),
            Some(176_400),
        ...
        extent.prove_end(100_000);
        assert_eq!(
            extent.frames(),
            Some(100_000),
            "what the source proved it can reach bounds what it claims"
        );
        extent.claim(Some(Duration::from_secs(8)), rate());
        assert_eq!(
            extent.frames(),
            Some(100_000),
