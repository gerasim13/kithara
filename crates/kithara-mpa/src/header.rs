...
mod bit_rates {
    pub(super) const INVALID_INDEX: u32 = 0xf;
    pub(super) const INVALID_L2_MONO: [u32; 4] = [224_000, 256_000, 320_000, 384_000];
    pub(super) const INVALID_L2_STEREO: [u32; 4] = [32_000, 48_000, 56_000, 80_000];
    /// Bit-rate lookup table for MPEG version 1 layer 1.
    pub(super) const MPEG1_L1: [u32; 15] = [
    ...
    pub(super) const MPEG1_L2: [u32; 15] = [
    ...
    pub(super) const MPEG1_L3: [u32; 15] = [
    ...
    pub(super) const MPEG2_L1: [u32; 15] = [
    ...
    pub(super) const MPEG2_L23: [u32; 15] = [
...
mod header_bits {
    pub(super) const BITRATE_MASK: u32 = 0xf000;
    pub(super) const BITRATE_SHIFT: u32 = 12;
    pub(super) const CHANNEL_MODE_MASK: u32 = 0xc0;
    pub(super) const CHANNEL_MODE_SHIFT: u32 = 6;
    pub(super) const CRC_MASK: u32 = 0x1_0000;
    pub(super) const LAYER_MASK: u32 = 0x6_0000;
    pub(super) const LAYER_SHIFT: u32 = 17;
    pub(super) const PADDING_MASK: u32 = 0x200;
    pub(super) const SAMPLE_RATE_MASK: u32 = 0xc00;
    pub(super) const SAMPLE_RATE_SHIFT: u32 = 10;
    pub(super) const SYNC_MASK: u32 = 0xffe0_0000;
    pub(super) const VERSION_MASK: u32 = 0x18_0000;
    pub(super) const VERSION_SHIFT: u32 = 19;
}
...
pub(crate) fn check_header(header: u32) -> bool {
    if (header & header_bits::VERSION_MASK) >> header_bits::VERSION_SHIFT == 0x1 {
        return false;
    }
    if (header & header_bits::LAYER_MASK) >> header_bits::LAYER_SHIFT == 0x0 {
        return false;
    }
    if (header & header_bits::BITRATE_MASK) >> header_bits::BITRATE_SHIFT
        == bit_rates::INVALID_INDEX
    {
        return false;
    }
    if SampleRates::get(
        MpegVersion::Mpeg1,
        (header & header_bits::SAMPLE_RATE_MASK) >> header_bits::SAMPLE_RATE_SHIFT,
    )
    .is_none()
    {
        return false;
...
pub(crate) fn is_frame_header_word_synced(sync: u32) -> bool {
    (sync & header_bits::SYNC_MASK) == header_bits::SYNC_MASK
}
...
pub(crate) fn parse_frame_header(header: u32) -> Result<FrameHeader> {
    ...
    let version = match (header & header_bits::VERSION_MASK) >> header_bits::VERSION_SHIFT {
    ...
    let layer = match (header & header_bits::LAYER_MASK) >> header_bits::LAYER_SHIFT {
    ...
    let bitrate_index = (header & header_bits::BITRATE_MASK) >> header_bits::BITRATE_SHIFT;
    let bitrate = match (bitrate_index, version, layer) {
        ...
        (i, MpegVersion::Mpeg1, MpegLayer::Layer1) => bit_rates::MPEG1_L1[i as usize],
        (i, MpegVersion::Mpeg1, MpegLayer::Layer2) => bit_rates::MPEG1_L2[i as usize],
        (i, MpegVersion::Mpeg1, MpegLayer::Layer3) => bit_rates::MPEG1_L3[i as usize],
        (i, _, MpegLayer::Layer1) => bit_rates::MPEG2_L1[i as usize],
        (i, _, _) => bit_rates::MPEG2_L23[i as usize],
    };
    ...
    let channel_mode = match (
        (header & header_bits::CHANNEL_MODE_MASK) >> header_bits::CHANNEL_MODE_SHIFT,
        layer,
    ) {
    ...
    if layer == MpegLayer::Layer2 {
        if channel_mode == ChannelMode::Mono {
            if bit_rates::INVALID_L2_MONO.contains(&bitrate) {
                return decode_error("mpa: invalid Layer 2 bitrate for mono channel mode");
            }
        } else if bit_rates::INVALID_L2_STEREO.contains(&bitrate) {
            return decode_error("mpa: invalid Layer 2 bitrate for non-mono channel mode");
        }
    }

    let has_padding = header & header_bits::PADDING_MASK != 0;

    let has_crc = header & header_bits::CRC_MASK == 0;
