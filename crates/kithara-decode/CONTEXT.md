...
`pts + dur` after each emitted packet) and sets `needs_resume` on an interrupted
read; the next `next_frame` re-seeks to `resume_ts` first so the interrupted
packet is re-read from its start. That re-seek is a bare position restore — no
pre-roll back-off, no codec flush (that is the user-seek path) — and is
idempotent when no strand occurred. `resume_ts` stays native: a `Duration`
round-trip loses sub-frame precision and snaps the re-seek one packet early.
