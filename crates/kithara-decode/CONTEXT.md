...
Detailed contracts and invariants for the kithara-decode crate; the README is the overview.
...
ramp for a duration nobody configured. `Duration::ZERO` is a valid
instantaneous change and takes the same path.
...
**A chunk's timestamp names the first emitted PCM frame it contains, not merely
the packet that caused the codec call.** A codec that buffers, delays, or trims
output must account for that before constructing the chunk. `frame_offset`
counts the same emitted PCM within that decoder generation.

Each decoder session owns its exact `DecoderTrackInfo`, including priming and
trailing-frame metadata, and applies its own `GaplessTrimmer`. Format-boundary
and seek replacement create a new session with a new local trim state; they
must not reuse static codec defaults or another generation's counters.
...
FLAC capability probing requires a synthetic STREAMINFO description. The
HE-AAC v2 browser decode fixture is generated by the ignored
`generate_he_aac_v2_fixture` native integration test.
