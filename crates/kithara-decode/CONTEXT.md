...
segment past the last one and a segment the layout has not described yet. Only
`ByteMap::len` states that the extent is final; over HLS it is published solely
when the variant's total is non-zero and every segment size is known.

`Fmp4SegmentDemuxer` therefore ends the stream only when the layout has stated
its extent, and parks on `PendingReason::Retry` otherwise. Reading an
