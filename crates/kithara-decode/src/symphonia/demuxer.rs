...
impl Demuxer for SymphoniaDemuxer {
    ...
    fn seek(&mut self, target: Duration, priming: CodecPriming) -> DecodeResult<DemuxSeekOutcome> {
        // WHY: park before target by max(priming warmup, one codec packet) so the trim guard lands on a packet boundary ([crate contracts](https://github.com/zvuk/kithara/wiki/kithara-decode) "Seek pre-roll and trim").
