...
use crate::{error::PlayError, player::PlayerImpl, session::PlayerLevel};
    ...
    S: 'a,
{
    ...
    for (i, &(engine, _)) in inputs.iter().enumerate() {
        if inputs[..i]
            .iter()
            .any(|&(other, _)| std::ptr::eq(other, engine))
        {
...
mod tests {
    ...
    fn apply_updates_desired_levels_for_two_players() {
        ...
        b.core.engine.start().unwrap();

        apply_mix([(&a, 0.25), (&b, 0.75)]).unwrap();
        assert_eq!(a.core.engine.master_volume(), 0.25);
        assert_eq!(b.core.engine.master_volume(), 0.75);
    }
    ...
    fn stopped_player_retains_level_on_restart() {
        ...
        a.core.engine.stop().unwrap();
        assert_eq!(a.core.engine.master_volume(), 0.4);
