...
fn catalog_env() -> &'static str {
    "KITHARA_FIXTURE_CATALOG"
}
...
fn codegen(
    ...
    for (index, (name, id, def)) in resolved.iter().enumerate() {
        ...
        let (cfg, body) = if embed && def.embed {
            (
                "",
                format!("    crate::asset::Asset::embedded(MANIFEST[{index}], include_bytes!({path:?}))"),
            )
        } else {
            (
...
fn write_catalog(
    ...
    let catalog = catalog_codegen(resolved, unavailable);
    let destination = std::env::var_os(catalog_env()).map_or_else(
        || {
            PathBuf::from(std::env::var_os("CARGO_MANIFEST_DIR").expect("cargo sets manifest dir"))
                .join("src/catalog.rs")
        },
        PathBuf::from,
    );
    if std::env::var_os(catalog_env()).is_some() {
        fs::write(&destination, catalog)
            .unwrap_or_else(|error| panic!("kithara-test-fixtures: write {}: {error}", destination.display()));
        return;
    }
    let committed = fs::read_to_string(&destination)
        .unwrap_or_else(|error| panic!("kithara-test-fixtures: read {}: {error}", destination.display()));
    if committed != catalog {
        panic!(
            "kithara-test-fixtures: committed catalog is stale; rebuild with {}=src/catalog.rs",
            catalog_env(),
        );
    }
}
...
fn catalog_codegen(
    ...
    for (name, id, def) in resolved {
        ...
        let _ = writeln!(
            entries,
            "    {name}: ({id:?}, {ext:?}, {content_type:?}, {unavailable});",
            ext = def.ext,
...
fn run() {
    ...
    fs::write(
        ...
        codegen(&namespace, &resolved, &unavailable, true),
    )
    .unwrap_or_else(|error| panic!("kithara-test-fixtures: write assets.rs: {error}"));
    write_catalog(&resolved, &unavailable);
}
