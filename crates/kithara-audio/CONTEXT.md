...
`Audio::new` builds the initial decoder **exactly once** (`create_initial_decoder` → one `spawn_blocking`), with **no retry loop and no readiness gate**. It reads through a reading session like every other decoder: `SharedStream::open_reader(ReaderHint { wait: WaitMode::Block })`. The wait is the **session's** property, not the stream's — that build runs off the produce core and may park, while the RT sessions opened later over the same stream keep probing. Two readers over one stream therefore answer "do I wait" differently, which is what lets a decoder be prepared off-core while another one is audible.

`WaitMode` governs both halves of the session: `Block` reads through the off-RT `Stream::read` adapter and seeks through the priming `Stream::seek`; `Probe` reads through `probe_read` and seeks through `probe_seek`, doing position math only. A session that waits for its bytes waits for them at a seek too.
