...
use super::{
    Consts, TimeStretchProcessor, chunk, f64_of, flush_serviced, process_serviced, processor, sine,
    spec,
};
use crate::{
    effects::timestretch::StretchControls,
    region::{GridSegment, RegionPlan},
    traits::AudioEffect,
};
...
fn exact_output_frames_do_not_drift_across_partitions() {
    ...
    let outputs = [1, 1, 1, 1].map(|frames| {
        let (output, next_remainder) = TimeStretchProcessor::output_frames(frames, 0.25, remainder)
...
fn pending_span_is_committed_before_live_unity_passthrough(#[case] backend: StretchKind) {
    ...
    let transition =
        process_serviced(&mut fx, unity).expect("rounded pending frame precedes the unity frame");
