...
pub(crate) trait WakeSignal: Send + Sync + 'static {
    ...
    /// Arm a consumer wake on every successful ring push. Delivery runs after
    /// the producer leaves the real-time core.
    fn wake(&self);
}
...
impl<T> Outlet<T> {
    ...
    /// when it was parked. Every successful push arms a consumer wake,
    /// coalesced and delivered after the producer pass.
