    ...
    S: HasPool<f32>,
{
    ...
    pub fn analyze(&self, mono_22050: &[f32]) -> Result<RawBeats, PoolError> {
        ...
        let mean = mean(&curve);
        let beats = decode::beats(&curve, &periods, self.tempo, &self.pools)?
            .iter()
            .copied()
            .map(|at| BeatMark {
...
fn confidence(curve: &[f32], at: f32, mean: f32) -> f32 {
    ...
    (1.0 / (1.0 + (-(value - mean) / mean).exp())).clamp(
        consts::tracker::CONFIDENCE_BOUNDS.0,
        consts::tracker::CONFIDENCE_BOUNDS.1,
    )
}
...
mod optimality {
    ...
    fn the_decoded_path_scores_the_dynamic_programs_optimum(clicks_120_20s: Vec<f32>) {
        ...
        let (decoded, optimum, hazards, observations, states) =
            decode::probe(&curve, &periods, Tempo::default(), &region)
                .expect("the probe fits the region");
