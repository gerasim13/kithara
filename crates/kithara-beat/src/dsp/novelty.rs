...
/// Complex spectral difference, one value per [`consts::frames::HOP`].
    ...
    S: HasPool<f32>,
{
    pub(crate) fn new(pools: PoolRegion<S>) -> Result<Self, PoolError> {
        let fft = RealFftPlanner::<f32>::new().plan_fft_forward(consts::frames::FRAME);
        Ok(Self {
    ...
    /// The difference is measured every [`consts::novelty::STRIDE`]
    /// samples and interpolated onto the [`consts::frames::HOP`] grid,
    ...
    pub(crate) fn curve(&self, mono: &[f32]) -> Result<SampleBuffer, PoolError> {
        if mono.len() < consts::frames::FRAME {
            return Ok(self.pools.get::<f32>());
        }
        let frames = (mono.len() - consts::frames::FRAME) / consts::novelty::STRIDE + 2;
        let mut coarse = self.pools.get_with_len::<f32>(frames)?;
        let bins = self.fft.complex_len();
        let mut work = Frames {
            input: self.pools.get_with_len::<f32>(consts::frames::FRAME)?,
        ...
        for (index, slot) in coarse.iter_mut().enumerate() {
            let at = index * consts::novelty::STRIDE;
            let end = (at + consts::frames::FRAME).min(mono.len());
        ...
        let mut curve = self.pools.get_with_len::<f32>(
            (coarse.len() - 1) * (consts::novelty::STRIDE / consts::frames::HOP) + 1,
        )?;
...
where
    S: HasPool<f32>,
{
    let mut hann = pools.get_with_len::<f32>(consts::frames::FRAME)?;
    let denom = (consts::frames::FRAME - 1).to_f32().unwrap_or(1.0);
    let scale = std::f32::consts::TAU / denom;
    for (n, sample) in hann.iter_mut().enumerate() {
        let phase = scale * n.to_f32().unwrap_or(0.0);
        *sample = consts::novelty::HANN_A0.mul_add(-phase.cos(), consts::novelty::HANN_A0);
    }
    Ok(hann)
}
...
mod tests {
    ...
    fn clicks_raise_peaks_where_the_clicks_are(clicks_120_4s: Vec<f32>) {
        ...
        let found: Vec<f32> = peaks(&curve)
            .into_iter()
            .map(|i| consts::frames::seconds(i.to_f32().unwrap_or(0.0)))
            .collect();
        // The click at zero lands on the first frame, where a peak has no
        // left neighbour to stand above.
        let first_observable = consts::frames::FRAME.to_f32().unwrap_or(0.0) / consts::frames::RATE;
