...
use super::{
    buffer::collected,
    consts::{DecodeConsts, PeriodConsts},
    tempo::Tempo,
};
    ...
    S: HasPool<f32>,
{
    ...
    let states = (longest + DecodeConsts::STATE_MARGIN * tempo.tolerance_frames())
        .floor()
        .to_usize()
        .unwrap_or(0);
...
where
    S: HasPool<f32>,
{
    let (beats, optimum) = decode(curve, periods, tempo, pools)?;
    let longest = periods
        .iter()
        .copied()
        .filter(|period| *period > 0.0)
    ...
    let hazards = periods
        .iter()
        .map(|&p| hazard(p, states, tempo, pools))
        .collect::<Result<_, _>>()?;
...
where
    S: HasPool<f32>,
{
    let hazard = hazard(period, states, tempo, pools)?;
    ...
    S: HasPool<f32>,
{
    ...
    let peak = DecodeConsts::DENSITY_SCALE / (tempo.tolerance() * std::f32::consts::TAU.sqrt());
