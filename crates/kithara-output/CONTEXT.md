...
`OfflineRenderConfig::block_frames` bounds work per renderer iteration.
`RenderSink` accepts interleaved `f32` blocks and has no finalize operation:
the composition owner finishes a concrete sink after a successful report and
drops it after cancellation or failure. `OfflineRenderReport::frames` counts
only frames delivered successfully to the sink.
