...
/// handle plus the key and URL for that variant's media `.m3u8`, tagged with
/// its [`VariantId`]. [`load`](Self::load) delegates to
...
impl VariantPlaylist {
    ...
    /// [`PlaylistCache::resolve_url`] and minting the resource key from `scope`.
