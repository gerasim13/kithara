...
use super::{HlsVariant, ReadLease, probe::SizeDemand};
...
impl HlsVariant {
    ...
    pub(super) fn all_sizes_complete(&self) -> bool {
        self.sizes_complete() && self.exact_init_complete()
    }

    pub(super) fn exact_init_complete(&self) -> bool {
        self.segments
            .init
            .as_ref()
    ...
    pub(super) fn exact_prefix_complete(&self, segment: u32) -> bool {
        if self
            .segments
            .init
            .as_ref()
...
impl ReadLease {
    pub(super) fn clear_exact_byte_seek(&self) {
        ...
        self.exact_byte_seek.store(None);
    }

    pub(super) fn clear_exact_seek(&self) {
        self.exact_seek.clear();
    }

    pub(super) fn complete_exact_seek_if_ready(&self) {
        let entry = {
            let Some(entry) = self.exact_seek.load() else {
                return;
            };
            if !self.variant.exact_prefix_complete(entry.segment) {
                return;
            }
            ...
            if !self.exact_seek.take_if(entry.generation) {
                return;
            }
            entry
        };
        ...
        let Some(exact_anchor) = self.variant.segment_byte_offset(demand.segment) else {
            return;
        };
        if self.cursor.position() == demand.anchor {
            ...
            self.cursor.set_position(exact_anchor);
    ...
    pub(super) fn exact_byte_metadata_phase(&self) -> Option<SourcePhase> {
        let byte = self.exact_byte_seek.load()?;
    ...
    fn exact_byte_position_ready(&self, byte: u64) -> bool {
        if self.variant.all_sizes_complete() {
            return true;
        }
        self.find_at_offset(byte)
            .is_some_and(|(segment, _, _)| self.variant.exact_prefix_complete(segment))
    }

    pub(super) fn exact_seek_metadata_phase(&self) -> Option<SourcePhase> {
        let demand = self.exact_seek.load()?;
        if self.variant.exact_prefix_complete(demand.segment) {
            return None;
        }
    ...
    pub(crate) fn prepare_exact_prefix_for_boundary(&self, boundary: u32) -> bool {
        ...
        let boundary = boundary.min(self.variant.num_segments());
        if self
            .variant
            .segments
            .init
        ...
        let Some(last) = boundary.checked_sub(1) else {
            return self.variant.exact_init_complete();
        };
        self.request_exact_prefix_through(last);
        self.variant.exact_prefix_complete(last)
    }
    ...
    fn request_exact_prefix_for_byte(&self, byte: u64) {
        ...
        let last = self.variant.num_segments().saturating_sub(1);
        if last > 0 || !self.variant.segments.is_empty() {
            self.request_exact_prefix_through(last);
        }
    }
    ...
    fn request_exact_prefix_through(&self, segment: u32) {
        ...
        let last = segment.min(self.variant.num_segments().saturating_sub(1));
    ...
    fn request_exact_size(&self, demand: SizeDemand) {
        trace!(
            ...
            demand = ?demand,
            probe_allowed = self.variant.size_probe_allowed(demand),
            "request exact size"
        );
    ...
    pub(super) fn set_exact_byte_seek_demand(&self, byte: u64) {
        if !needs_exact_byte_sizes(self.variant.profile.codec, self.variant.profile.container)
            || self.variant.all_sizes_complete()
        {
            self.clear_exact_byte_seek();
            return;
        }
        ...
        self.exact_byte_seek.store(Some(byte));
        self.request_exact_prefix_for_byte(byte);
    }
    ...
    pub(super) fn set_exact_seek_demand(&self, anchor: u64, segment: u32) {
        ...
        self.exact_seek.set(segment, anchor);
        self.request_exact_prefix_through(segment);
        self.complete_exact_seek_if_ready();
    }
}
