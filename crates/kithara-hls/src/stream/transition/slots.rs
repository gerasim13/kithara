...
pub(in crate::stream) struct SessionSlots<S>
where
    S: HasPool<u8> + Send + Sync + 'static,
{
    publication: ArcSwap<ResidentSessions<S>>,
    pub(super) transition: Mutex<TransitionState<S>>,
}
...
where
    S: HasPool<u8> + Send + Sync + 'static,
{
    pub(in crate::stream) fn new(active: Arc<HlsSession<S>>) -> Self {
        Self {
    ...
    pub(in crate::stream) fn active(
        &self,
        mut selected_variant: impl FnMut() -> usize,
    ) -> Arc<HlsSession<S>> {
        loop {
    ...
    pub(in crate::stream) fn incoming_session(&self) -> Option<Arc<HlsSession<S>>> {
    ...
    pub(super) fn publish_exact_two(
        &self,
    ...
    #[cfg(test)]
    pub(in crate::stream) fn resident_count(&self) -> usize {
...
pub(super) struct TransitionState<S>
where
    S: HasPool<u8> + Send + Sync + 'static,
{
...
where
    S: HasPool<u8> + Send + Sync + 'static,
{
    pub(super) session: Arc<HlsSession<S>>,
    pub(super) landing_time: Duration,
    pub(super) reader: Option<OpenedVariantReader>,
    pub(super) claim: PendingAbrDecision,
    pub(super) profile: ReaderProfile,
...
where
    S: HasPool<u8> + Send + Sync + 'static,
{
    pub(in crate::stream) fn abort_variant(&self, transition: VariantTransition) -> bool {
    ...
    pub(in crate::stream) fn cancel_incoming_for_seek(&self) {
        let mut state = self.sessions.transition.lock();
    ...
    pub(in crate::stream) fn plan_variant_reader(
        &self,
    ...
    pub(in crate::stream) fn promote_planned_variant(
        &self,
        transition: VariantTransition,
    ) -> VariantPromotion {
        let mut state = self.sessions.transition.lock();
...
pub(super) fn unsupported_pending_claim() -> StreamError {
