use super::{
    ...
    BeatGridStamp, BeatGridState, BeatGridUnavailable, BeatGridView, view::stale,
};
use crate::{
    AssetFrame, Beat, BeatAlignment, BeatsPerMinute, MapAxis, MapPoint, MapPosition, MapRegion,
    Meter,
};
...
pub enum GridProjectionError {
    /// An alignment edge names a grid revision other than the one it relates.
    #[error("the alignment names {given:?}, but the grid carries {expected:?}")]
    Foreign {
        expected: BeatGridStamp,
        given: BeatGridStamp,
    },
    /// The target grid places nothing at the beat the alignment names.
    #[error("the target grid places no position at the aligned beat")]
    Unalignable,
...
impl GridProjection {
    pub(super) fn new(
        ...
        own(&source, alignment.source().stamp())?;
        own(&target, alignment.target().stamp())?;
        if !matches!(
            target.position_at(alignment.target()),
...
/// Refuses an alignment edge that names another revision of the grid.
fn own(grid: &BeatGridSnapshot, given: BeatGridStamp) -> Result<(), GridProjectionError> {
    let expected = grid.stamp();
    if given == expected {
        Ok(())
    } else {
        Err(GridProjectionError::Foreign { expected, given })
    }
}
...
mod tests {
    ...
    /// The cue beat of the recording, aligned with the Host beat sounding at
    ...
    fn alignment(
        ...
        BeatAlignment::new(
            ...
            MapPoint::new(
                target.stamp(),
                Beat::new(host_beat).expect("invariant: fixture beat is finite"),
            ),
        )
    }

    fn projection(bpm: f64, cue: f64, activation: i64) -> GridProjection {
        let (source, target) = (source(bpm), host());
        let alignment = alignment(&source, &target, cue, activation);
        GridProjection::new(source, target, alignment)
    ...
    fn a_projection_refuses_a_pair_of_grids_it_cannot_relate() {
        let (live, recording) = (host(), source(124.0));

        assert_eq!(
            GridProjection::new(host(), host(), alignment(&live, &live, 0.0, 0)).err(),
        ...
        assert_eq!(
            GridProjection::new(
                source(124.0),
                source(124.0),
                alignment(&recording, &recording, 0.0, 0)
            )
            .err(),
    ...
    #[kithara::test]
    fn a_projection_refuses_an_alignment_that_names_another_grid() {
        let (source, target) = (source(124.0), host());
        let foreign = alignment(&source, &host(), 0.0, 0);
        let expected = target.stamp();
        let given = foreign.target().stamp();

        assert_eq!(
            GridProjection::new(source, target, foreign).err(),
            Some(GridProjectionError::Foreign { expected, given }),
            "an alignment edge relates the two grids it names, never a third"
        );
    }
    ...
    #[kithara::test]
    fn a_projection_refuses_a_target_that_places_no_beat_at_the_alignment() {
        let source = source(124.0);
        let target = BeatGridSnapshot::unavailable(
            BeatGridId::allocate().expect("invariant: fixture grid id can be allocated"),
            BeatGridRevision::first(),
            MapAxis::Session(SessionAxis::new(sample_rate(), SessionEpoch::new(0))),
        );
        let alignment = alignment(&source, &target, 0.0, 96_000);

        assert_eq!(
            GridProjection::new(source, target, alignment).err(),
