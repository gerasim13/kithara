...
const DEFAULT_ARTIFACT_DIR_ENV: &str = "KITHARA_AUDIO_ARTIFACT_DIR";
...
/// Write listening WAV files and a manifest when `artifact_dir_env` is set.
/// Returns `Ok(None)` when unset.
///
/// # Errors
/// Returns an error for invalid audio, serialization, or filesystem failure.
pub fn write_audio_artifact_from_env<T: Serialize>(
    artifact_dir_env: &str,
    case: &str,
    sample_rate: u32,
    channels: u16,
    audio: &[(&str, &[f32])],
