...
use super::{super::device::Selected, build, runner};
use crate::child;

pub(super) fn art_nextest_list_extra(target: &str, packages: &BTreeSet<String>) -> Vec<String> {
    [
        vec![
            "--no-default-features".into(),
            "--features".into(),
            super::super::device_features(crate::BuildProfile::Debug),
            "--tests".into(),
            "--target".into(),
            target.into(),
            "--profile".into(),
...
pub(super) fn android_product_packages(
    root: &Path,
    target: &str,
    product: &str,
    ...
    let output = Command::new("cargo")
        ...
        .args([
            ...
            "--no-default-features",
            "--features",
            &super::super::device_features(crate::BuildProfile::Release),
            "--edges",
            "normal",
            "--prefix",
            "none",
...
pub(super) fn command_packages(command: &Command) -> Result<BTreeSet<String>> {
...
pub(crate) struct Prepared {
    ...
    pub(super) session: runner::Session,
    pub(super) _cache_lease: FileLock,
}
