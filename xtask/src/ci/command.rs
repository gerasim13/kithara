...
use super::{bridge::BridgeArgs, host::HostArgs, run::RunArgs};
...
pub(crate) fn is_standalone(args: &CiArgs) -> bool {
    matches!(args.command, CiCommand::Bridge(_) | CiCommand::Host(_))
}
...
pub(crate) fn run(args: &CiArgs, ctx: &Ctx) -> Result<()> {
    match &args.command {
        CiCommand::Bridge(_) | CiCommand::Host(_) => run_standalone(args),
